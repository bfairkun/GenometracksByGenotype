...
colorbrewer.RdPrBlu = {3: [(240,59,32), (122,1,119), (8,81,156)], 4: [(240,59,32), (122,1,119), (8,81,156), (189,189,189)]}
...
def setup_logging(verbosity):
    ...
    logging.basicConfig(level=loglevel, format="[%(asctime)s] {%(pathname)s:%(lineno)d} %(levelname)s - %(message)s")
...
def ConstructSamplesTsvFromGlobPattern(GlobPatternString, vcf_fn, fn_out=None):
    ...
    for bigwig_fn in BigwigGlobmatchs:
        ...
        if len(Match) == 1: query = Match[0]
        else: continue
        vcf_sample_matches = [i for i in vcf_samples if query in i]
        if len(vcf_sample_matches) == 1: vcf_sample_match = vcf_sample_matches[0]
        else: continue
        BigwigFile_VcfSample_Matchlist.append([vcf_sample_match, bigwig_fn])
    df = pd.DataFrame(BigwigFile_VcfSample_Matchlist, columns=["SampleID", "BigwigFilepath"])
    df["Group_label"] = ""
    df["Strand"] = ""
    if fn_out:
        pass
...
def AppendGenotypeColumnToDf(df, vcf_fn=None, SnpPos=None):
    ...
    homo_alts = []
    hets = []
    for record in vcf_reader.fetch(chrom=SnpChr, start=(str_to_int(SnpCoord)-1), end=str_to_int(SnpCoord)):
...
def FillColorsIfNoneProvided(df, FactorColumnName, GroupColorColumnName='Group_color', OutputColumnName='color', ColorPallette="Dark2"):
    df[GroupColorColumnName] = df[GroupColorColumnName].replace(r'^\s*$', numpy.nan, regex=True)
    if df[GroupColorColumnName].isnull().values.all():
        return AssignColorsFromFactorColumn(df=df, FactorColumnName=FactorColumnName, OutputColumnName=OutputColumnName, ColorPallette=ColorPallette)
    else:
    ...
    for i, row in df.iterrows():
        ...
        for j, bigwig in enumerate(row[BigwigFilesInColumnName]):
            ...
            if (not dryrun) and BigwigFilesOutPerSampleColumnName:
                try:
                    bwOut = pyBigWig.open(row[BigwigFilesOutPerSampleColumnName][j], "w")
        ...
        if not dryrun:
            ...
            bwOut.addHeader(list(bw.chroms().items()))
            bwOut.addEntries(RegionChr, RegionStart, values=ArrayAveraged, span=1, step=1)
            bwOut.close()
...
def PysamTabixToPandas_df(fn_in, Chrom=None, start=None, end=None, **kwargs):
    ...
    RowsInRegion.seek(0)
    # print(JuncsInRegion.read())
    return pd.read_csv(RowsInRegion, sep='\t', **kwargs)


def NormalizeAverageAndWriteOutLinks(df, Region, FilterForJuncsBed=None, dryrun=False, BedgzFilesInColumnName='BedgzFilepath', SampleIDColumnName='SampleID', StrandColumnName='Strand', LinksFilesOutColumnName='links_out', OutputStringFormat="{0:.2g}", SwapStrandFilters=False, MinPSI=0):
    """
...
def parse_args(Args=None):
    ...
    p.add_argument("--SnpPos",
            ...
            metavar="<CHR>:<POS>",
            default=None)
    p.add_argument('--BigwigListType', choices=["KeyFile", "GlobPattern"], help='Required. Define how the bigwig list positional argument is specified. The GlobBattern option requires use of --VCF option', required=True)
...
def main(**kwargs):
    ...
    if kwargs['BigwigListType'] == 'KeyFile':
        ...
        if kwargs['VCF']:
            logging.info("MakeKeyFile")
            DF = ConstructSamplesTsvFromGlobPattern(kwargs["BigwigList"], kwargs['VCF'])
        else: raise Exception("Files bigwig files as a glob pattern, must supply a VCF")
    logging.debug("Adding genotype to DF")
    DF = AppendGenotypeColumnToDf(DF, vcf_fn=kwargs["VCF"], SnpPos=kwargs['SnpPos'])
    logging.debug("Adding column for individual output bigwigs filepaths to DF")
    DF['bw_out_PerInd'] = kwargs['OutputPrefix'] + DF[['SampleID' ,'Group_label', 'genotype', 'Strand']].agg('-'.join, axis=1) + '.bw'
    # groubpy
    DF = DF.groupby([col for col in DF.columns if col not in [ 'BigwigFilepath', 'SampleID', 'bw_out_PerInd']], as_index=False)[['BigwigFilepath', 'bw_out_PerInd', 'SampleID']].agg(lambda x: list(x))
    # Add per group setting columns
    DF = AddExtraColumnsPerGroup(DF, GroupToBedTsv=kwargs['GroupSettingsFile'], BedfileForAll=kwargs['BedfileForSashimiLinks'])
    ...
    logging.debug("Writing normalized and averaged bigwigs")
    ...
    if kwargs['TracksTemplate']:
        ...
        with open(GetDefaultTemplate(DF), 'r') as fh:
            template = Template(fh.read())
    logging.debug("Writing template with dataframe with the following df.columns accessible in jinja2 template: {}".format(DF.columns))
